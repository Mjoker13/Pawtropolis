...
public interface ZooController {
    ...
    //Metodo per calcolare chi ha la coda piu lunga
    /*public static Animal getgetLongestTail(List<Integer> longestTail){
        int l=0;
        for (int i = 0; i < longestTail.size()-1; i++) {
            if(longestTail.get(i) < longestTail.get(i+1)){
                l=longestTail.get(i);
            }else l= longestTail.get(i+1);
        }
    }*/
}
